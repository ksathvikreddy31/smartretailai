...
# # Paths
# MODEL_PATH = os.path.normpath(os.path.join(BASE_DIR, "../../../ai-layer/forecasting/forecast_model.pkl"))
# PRODUCTS_CSV = os.path.normpath(os.path.join(BASE_DIR, "../../../products.csv"))
# SALES_CSV = os.path.normpath(os.path.join(BASE_DIR, "../../../sales.csv"))
...
#     """
#     Generate future forecasts dynamically using the trained Prophet model.
#     Distributes the aggregate forecast among products based on historical sales share.
#     """
#     if model is None:
...
#         # Calculate historical weights for each product
#         total_sales_quantity = sales_df['quantity_sold'].sum()
#         product_weights = sales_df.groupby('product_id')['quantity_sold'].sum() / total_sales_quantity
...
#         # Get total forecast for next 7 days and 30 days
#         # Prophet returns historical + future. We want the future 'days' rows.
#         future_forecast = forecast.tail(days)
        
#         # total 7-day forecast (sum of yhat for the first 7 days of the future period)
#         total_7_days = future_forecast.head(7)['yhat'].sum()
#         # total 30-day forecast
#         total_30_days = future_forecast['yhat'].sum()

#         # Calculate trend
#         # Compare first 7 days of forecast vs last 7 days of training data
#         historical_last_7 = forecast.iloc[-(days+7):-days]['yhat'].sum()
#         overall_trend = "up" if total_7_days > historical_last_7 else "down"

#         results = []
#         for _, product in products_df.iterrows():
#             pid = product['product_id']
#             weight = product_weights.get(pid, 0.0) # If no sales history, weight is 0
            
#             f7 = round(total_7_days * weight)
#             f30 = round(total_30_days * weight)
            
#             results.append({
#                 "product": product['product_name'],
#                 "category": product['category'],
#                 "current_stock": int(product['stock']),
#                 "forecast_7_days": int(f7),
#                 "forecast_30_days": int(f30),
#                 "trend": overall_trend
#             })

#         # Sort products by highest forecast demand (30 days)
#         results.sort(key=lambda x: x['forecast_30_days'], reverse=True)

#         # Return TOP 5 forecast products
#         return results[:5]

#     except Exception as e:
#         print(f"Exception in predict_demand: {e}")
#         return []
